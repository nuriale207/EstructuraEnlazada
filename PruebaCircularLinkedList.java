
import java.util.Iterator;


public class PruebaCircularLinkedList {
	
	public static void visualizarNodos(UnorderedCircularLinkedList<Integer> l) {
		Iterator<Integer> it = l.iterator();
		System.out.println();
		while (it.hasNext()) {
			Integer num = it.next();
			System.out.println(num);
		}
	}
	
	
	
	
	public static void main(String[] args)  {
		
		UnorderedCircularLinkedList<Integer> l = new UnorderedCircularLinkedList<Integer>();
		l.addToRear(1); 
		l.addToRear(3);
		l.addToRear(6);
		l.addToRear(7);
		l.addToRear(9);
		l.addToRear(0);
		l.addToRear(20);
		l.addToFront(8);
		l.addAfter(20, 20);
		l.addAfter(9, 20);
		l.addAfter(7, 8);
		l.addAfter(2, 1);
		l.addAfter(5, 15);
		visualizarNodos(l);
		UnorderedCircularLinkedList<Integer> l2 = new UnorderedCircularLinkedList<Integer>();
		l2.addToRear(8);
		visualizarNodos(l2);
		l2.remove(new Integer(8) );
		visualizarNodos(l2);
		
		
		
		System.out.print(" Lista ...............");
		visualizarNodos(l);
		System.out.println(" Num elementos: " + l.size());
		
		System.out.print(" Prueba Eliminar ...............");
		l.removeLast();
		l.removeFirst();
		l.remove(3);
		visualizarNodos(l);
		System.out.println(" Num elementos: " + l.size());
		
		System.out.println("Prueba Find ...............");
		System.out.println("9? " + l.find(9));
		System.out.println("20? " + l.find(20));
		System.out.println("0? " + l.find(0));
		System.out.println("7? " + l.find(7));
		System.out.println("3? " + l.find(3));
		System.out.println("6? " + l.find(6));
		System.out.println("100? " + l.find(100));
		
}
}
